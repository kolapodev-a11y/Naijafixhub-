...
import {
  FiCheck, FiX, FiFlag, FiUsers, FiList, FiAlertTriangle,
  FiRefreshCw, FiEye, FiShield, FiBell, FiSearch
} from 'react-icons/fi'
...
export default function AdminDashboard() {
  ...
  const [rejectModal, setRejectModal] = useState(null) // artisanId
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">PeezuTech — Manage NaijaFixHub</p>
        </div>
        <button onClick={fetchAll} disabled={loading}
          className="btn-ghost flex items-center gap-1.5 border border-gray-200 text-sm">
        ...
        {TABS.map(tab => (
          ...
          >
            ...
            {tab === 'Pending' && stats.pending > 0 && (
              <span className="ml-1.5 bg-orange-500 text-white text-xs rounded-full px-1.5">{stats.pending}</span>
            )}
            {tab === 'Reports' && stats.reports > 0 && (
              <span className="ml-1.5 bg-red-500 text-white text-xs rounded-full px-1.5">{stats.reports}</span>
            )}
          </button>
        ))}
      </div>
                  ...
                  <div key={listing._id} className="card p-5">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                      ...
                      <div className="flex flex-col gap-2 flex-shrink-0">
                        ...
                        <button onClick={() => setRejectModal(listing._id)}
                          className="flex items-center gap-1.5 border border-red-200 text-red-500 hover:bg-red-50 text-sm font-semibold px-4 py-2 rounded-xl transition-colors">
                          <FiX size={14} /> Reject
                        </button>
                      </div>
                    </div>
      ...
      {activeTab === 'Reports' && (
        <div className="space-y-4">
          {data.reports.length === 0 ? (
            ...
            data.reports.filter(r => r.status !== 'resolved').map(report => (
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex-1">
                    <p className="font-semibold text-gray-800 flex items-center gap-2">
                      ...
                      Report on: <span className="text-primary-600">{report.artisan?.title || 'Unknown Artisan'}</span>
                    </p>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Reported by: {report.user?.email || 'Anonymous'} • {timeAgo(report.createdAt)}
                    </p>
                  </div>
                  <div className="flex gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => handleResolveReport(report._id, 'ban')}
                      className="bg-red-500 hover:bg-red-600 text-white text-sm font-medium px-3 py-2 rounded-xl transition-colors">
                      Take Action
                    </button>
                  </div>
                </div>
          ...
          <div className="mb-4 flex items-center gap-3">
            <div className="flex-1 relative">
              ...
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                className="input-field pl-9 text-sm py-2.5" placeholder="Search listings..." />
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-500 uppercase text-xs">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left hidden lg:table-cell">Posted</th>
                </tr>
              </thead>
                ...
                {data.pending.map(l => (
                  <tr key={l._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <p className="font-medium text-gray-800 line-clamp-1">{l.title}</p>
                      <p className="text-xs text-gray-400">{l.user?.name}</p>
                    </td>
                    <td className="px-4 py-3 hidden sm:table-cell text-gray-500">
                      {CATEGORIES.find(c=>c.id===l.category)?.name || l.category}
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell text-gray-500">{l.location}</td>
                    <td className="px-4 py-3">
                      <span className={l.status === 'approved' ? 'badge-verified' : l.status === 'rejected' ? 'badge-rejected' : 'badge-pending'}>
                        {l.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 hidden lg:table-cell text-gray-400">{timeAgo(l.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Modal isOpen={!!rejectModal} onClose={() => setRejectModal(null)} title="❌ Reject Listing">
        <div className="space-y-4">
          ...
          <textarea
            ...
            placeholder="e.g. 'Profile photo is unclear', 'Description contains misleading info'..."
          />
          <div className="flex gap-3">
            <button onClick={() => { setRejectModal(null); setRejectReason('') }}
              className="flex-1 btn-ghost border border-gray-200">Cancel</button>
