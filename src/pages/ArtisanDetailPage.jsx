...
export default function ArtisanDetailPage() {
  ...
  const waLink = generateWhatsAppLink(artisan.whatsapp || '', `Hi ${artisan.title}, I found you on NaijaFixHub and I need your services.`)
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card p-6">
            <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
              <div>
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full text-white" style={{ backgroundColor: category?.color || '#7C3AED' }}>{category?.icon} {category?.name || artisan.category}</span>
                <h1 className="text-2xl font-extrabold text-gray-800 mt-2">{artisan.title}</h1>
                <p className="flex items-center gap-1.5 text-gray-500 text-sm mt-1"><FiMapPin size={14} /> {artisan.location}</p>
              </div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-xs text-gray-400 flex items-center gap-1"><FiClock size={11} /> Listed {timeAgo(artisan.createdAt)}</span>
            </div>

            <div className="prose prose-sm max-w-none text-gray-600 mb-4"><p>{artisan.description}</p></div>
            {artisan.serviceArea && <div className="bg-primary-50 rounded-xl p-3 text-sm text-primary-800"><strong>Service Area:</strong> {artisan.serviceArea}</div>}
          </div>

          <div className="card p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2"><FiStar className="text-yellow-400" /> Reviews ({reviews.length})</h2>
              {isAuthenticated && !isOwnListing && <button onClick={() => setReviewModal(true)} className="btn-outline text-sm py-2">Write Review</button>}
            </div>
                ...
                {reviews.map((review) => (
                  <div key={review._id} className="border-b border-gray-50 last:border-0 pb-4 last:pb-0">
                    <div className="flex items-start gap-3">
                      <div className="w-9 h-9 bg-gradient-to-br from-primary-400 to-accent-400 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0">{getInitials(review.user?.name || 'U')}</div>
        ...
        <div className="space-y-5">
          <div className="card p-5 sticky top-20">
            ...
            <a href={waLink} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white font-bold py-3.5 rounded-xl w-full transition-colors mb-3 text-sm"><FaWhatsapp size={18} /> Chat on WhatsApp</a>
            <button onClick={() => setContactModal(true)} className="flex items-center justify-center gap-2 btn-outline w-full text-sm"><FiSend size={15} /> Send a Message</button>
            ...
            <button onClick={() => setReportModal(true)} className="flex items-center justify-center gap-2 w-full mt-3 text-red-500 hover:text-red-700 hover:bg-red-50 py-2 rounded-xl text-xs font-medium transition-colors"><FiFlag size={13} /> Report This Artisan</button>
          </div>
          ...
          <div className="card p-5">
            ...
            <div className="space-y-2.5 text-sm">
              ...
              {artisan.isPremium && <div className="flex justify-between"><span className="text-gray-400">Plan</span><span className="font-medium text-yellow-600 flex items-center gap-1"><FaCrown size={12} /> Premium</span></div>}
            </div>
          </div>
        </div>
      </div>
      ...
      <Modal isOpen={reportModal} onClose={() => setReportModal(false)} title="🚨 Report Artisan">
        <form onSubmit={handleReport(onReport)} className="space-y-4">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700">All reports are reviewed by our admin team. False reports may result in account suspension.</div>
      ...
      <Modal isOpen={contactModal} onClose={() => setContactModal(false)} title="📬 Send a Message">
        <div className="space-y-4">
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm text-blue-800">NaijaFixHub recommends using WhatsApp for the fastest response.</div>
          <a href={waLink} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 bg-green-500 text-white font-bold py-3 rounded-xl w-full hover:bg-green-600 transition-colors"><FaWhatsapp size={18} /> Open WhatsApp Chat</a>
