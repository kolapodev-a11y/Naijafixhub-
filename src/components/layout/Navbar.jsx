...
import {
  ...
  FiSettings, FiPlusCircle, FiShield, FiBell
} from 'react-icons/fi'
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            <NavLink to="/" end className={({ isActive }) =>
              `px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:text-primary-700 hover:bg-primary-50'}`
            }>Home</NavLink>
            <NavLink to="/search" className={({ isActive }) =>
              `px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:text-primary-700 hover:bg-primary-50'}`
            }>Browse Services</NavLink>
            <NavLink to="/post-service" className={({ isActive }) =>
              `px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:text-primary-700 hover:bg-primary-50'}`
            }>Offer a Service</NavLink>
            {isAdmin && (
              <NavLink to="/admin" className={({ isActive }) =>
                `flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-purple-100 text-primary-700' : 'text-primary-600 hover:bg-purple-100'}`
              }>
                <FiShield size={14} />
                Admin
                {pendingCount > 0 && (
          ...
          <div className="flex items-center gap-2">
            ...
            {isAuthenticated ? (
              <>
                <Link
                  to="/post-service"
                  className="hidden sm:flex btn-accent text-sm py-2 px-4 items-center gap-1.5"
                >
                ...
                <div className="relative" ref={dropRef}>
                  <button
                    ...
                    className="flex items-center gap-2 p-1 rounded-xl hover:bg-primary-50 transition-colors"
                  >
                  ...
                  {dropdownOpen && (
                    <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-xl border border-gray-100 py-1 z-50">
                      ...
                      <Link to="/my-listings" onClick={() => setDropdownOpen(false)}
                        className="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-primary-50 hover:text-primary-700 transition-colors">
                        <FiSettings size={15} /> My Listings
                      </Link>
                      {isAdmin && (
                        <Link to="/admin" onClick={() => setDropdownOpen(false)}
                          className="flex items-center gap-2 px-4 py-2.5 text-sm text-primary-600 font-medium hover:bg-primary-50 transition-colors">
                          <FiShield size={15} />
                          Admin Dashboard
            ...
            {/* Mobile menu toggle */}
            <button
              className="md:hidden p-2 text-gray-500 hover:text-primary-600 rounded-lg transition-colors"
      ...
      {menuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-3 space-y-1">
          <NavLink to="/" end onClick={() => setMenuOpen(false)}
            className={({ isActive }) => `block px-3 py-2 rounded-lg text-sm font-medium ${isActive ? 'bg-primary-50 text-primary-700' : 'text-gray-700'}`}>
            Home
          </NavLink>
          ...
          <NavLink to="/post-service" onClick={() => setMenuOpen(false)}
            className={({ isActive }) => `block px-3 py-2 rounded-lg text-sm font-medium ${isActive ? 'bg-primary-50 text-primary-700' : 'text-gray-700'}`}>
            Offer a Service
          </NavLink>
          {isAdmin && (
            <NavLink to="/admin" onClick={() => setMenuOpen(false)}
              className={({ isActive }) => `flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium text-primary-600 ${isActive ? 'bg-purple-50' : ''}`}>
