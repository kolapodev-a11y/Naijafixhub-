...
export default function SmartMatchForm() {
  const { isAuthenticated, canRequestServices, accountType } = useAuth()
  ...
  const onSubmit = async (data) => {
    ...
    if (!canRequestServices) {
      toast.error('Your current account type cannot post service requests. Switch to Customer or Both in your profile.')
      navigate('/profile')
      return
    }
    ...
    return (
      <div className="overflow-hidden rounded-2xl bg-white shadow-card">
        <div className="bg-gradient-to-r from-primary-600 to-accent-500 p-5">
          ...
          <p className="mt-1 text-sm text-white/80">Only signed-in users can post service requests and receive direct responses.</p>
        </div>
  ...
  if (!canRequestServices) {
    return (
      <div className="overflow-hidden rounded-2xl bg-white shadow-card">
        <div className="bg-gradient-to-r from-primary-600 to-accent-500 p-5">
          <h2 className="flex items-center gap-2 text-xl font-bold text-white">⚡ Smart Match – Post Your Need</h2>
          <p className="mt-1 text-sm text-white/80">Your account is currently set to {accountType}. Update it anytime in your profile.</p>
        </div>
        <div className="space-y-4 p-6">
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            Provider-only accounts cannot post service requests. Switch your account type to Customer or Both to use Smart Match.
          </div>
          <Link to="/profile" className="btn-primary inline-flex">Go to Profile Settings</Link>
        </div>
      </div>
    )
  }
  ...
  if (submitted) {
    return (
      <div className="rounded-2xl bg-white p-8 text-center shadow-card">
        ...
        <button onClick={() => setSubmitted(false)} className="btn-primary">Post Another Request</button>
      </div>
    )
  }
    ...
    <div className="overflow-hidden rounded-2xl bg-white shadow-card">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 p-5">
        ...
        <div>
          ...
          <textarea {...register('description')} rows={3} className={`input-field resize-none ${errors.description ? 'border-red-300 ring-2 ring-red-300' : ''}`} placeholder='e.g. "I need a plumber to fix a leaking pipe urgently"' />
          {errors.description && <p className="mt-1 flex items-center gap-1 text-xs text-red-500"><FiAlertCircle size={12} /> {errors.description.message}</p>}
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            {errors.urgency && <p className="mt-1 flex items-center gap-1 text-xs text-red-500"><FiAlertCircle size={12} /> {errors.urgency.message}</p>}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input {...register('whatsapp')} className={`input-field pl-9 ${errors.whatsapp ? 'border-red-300 ring-2 ring-red-300' : ''}`} placeholder="e.g. 08012345678" type="tel" />
          </div>
          {errors.whatsapp && <p className="mt-1 flex items-center gap-1 text-xs text-red-500"><FiAlertCircle size={12} /> {errors.whatsapp.message}</p>}
